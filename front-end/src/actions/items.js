...
// GET ITEMS (demographic)
export const getDemographicItem = (location) => dispatch  => {
    axios.get(`${host_url}/api/items/items_demographic/${location}/`)
        .then(res => {
            dispatch({
                type: GET_ITEMS_DEMOGRAPHIC,
                payload: res.data
            });
        })
        .catch(err => console.log(err))
