...
class ItemView(viewsets.ModelViewSet):
    ...
    @staticmethod
    def top_places_by_location(location):
        # Filter the DataFrame for the given location
        file_path = os.path.join(os.getcwd(), 'trip_planning', 'result.xlsx')
        result = pd.read_excel(file_path)

        location_df = result[result['Location'] == location.title()]
        # Sort the DataFrame by the weighted rating (WR) in descending order
        sorted_df = location_df.sort_values(by='WR', ascending=False)
        # Initialize a set to store unique places
        unique_places = set()
        # Initialize a list to store the top places
        top_places = []
        # Iterate over the sorted DataFrame
        for _, row in sorted_df.iterrows():
            place = row['Place']
            if place not in unique_places:
                top_places.append({'Place': place, 'WR': row['WR']})
                unique_places.add(place)

            # Break the loop if we have 10 unique places
            if len(top_places) >= 10:
                break

        # Convert the list of top places to a DataFrame
        top_places_df = pd.DataFrame(top_places)

        return top_places_df

    @action(detail=False, url_path=r'items_demographic/(?P<location>\w+)')
    def items_demographic(self, request, location=None):
        try:
            # Get the top places for the given location
            top_places_df = self.top_places_by_location(location)

            # Get the names of the top places
            print('################################################')
            print('################################################')
            print('##############    TO COMPLETE    ###############')
            print('################################################')
            ...
            # top_place_names = top_places_df['Place'].tolist()
            top_place_names = ['Museum Theater Memory', 'Museum of Cinema']

            # Filter your Django model queryset using the names of the top places
            queryset = Item.objects.filter(name__in=top_place_names)
